...
/**
 * <b>AEtoile</b> représente l'algoritheme A* avec une heuristique différente.
 * CLÉ : NOMBRE DE CASES MAL PLACÉES + NOMBRE DE MOUVEMENTS
 * DISTANCE : NOMBRE DE MOUVEMENTS
 */
public class AEtoile extends Algorithme {
	 ...
	 * @param p
	 */
	public AEtoile(Plateau p) {
		super(p);
	}
	
	@Override
	public LinkedList<Plateau> algorithme() {
		file.insertion(s, s.getP().distanceAvecFinale());
		nbElemDansFile++;
		while(!file.estVide()) {
			...
			for(Plateau v: fils) {
				...
				if(file.indiceDansFile(tmp) != -1) {
					...
					if(tmp.getMouv() > u.getMouv()+1) {
						...
						tmp.setPere(u);
						file.MaJ(tmp, tmp.getP().distanceAvecFinale() + tmp.getMouv());
					}
				}else if(vu.containsKey(tmp)) {
					int dist = vu.get(tmp);
					if(dist > u.getMouv()+1) {
						vu.remove(tmp);
						file.insertion(tmp, v.distanceAvecFinale() + tmp.getMouv());
						nbElemDansFile++;
					}
				}			
				else {
